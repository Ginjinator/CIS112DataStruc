...
*/
    ...
    public T[] getObjectArray() {
        ...
        LinkedQueue<T> queue = this.getNodesAscending();
        int i = 0;
        while (!queue.isEmpty()) {
    ...
    public LinkedQueue<T> getNodesAscending() {
        LinkedQueue<T> out = new LinkedQueue<>();
        Iterator iter = this.getIterator(Traversal.Inorder);
        while (iter.hasNext()) {
            out.enqueue((T) iter.next());
        }
        return out;
    }

    public LinkedQueue<T> getNodesDescending() {
        LinkedQueue<T> out = new LinkedQueue<>();
        Iterator iter = this.getIterator(Traversal.Inorder);
        LinkedStack<T> stack = new LinkedStack<>();
        while (iter.hasNext()) {
            stack.push((T) iter.next());
        }
        while (!stack.isEmpty()) {
            out.enqueue(stack.top());
            stack.pop();
        }
        return out;
    }
